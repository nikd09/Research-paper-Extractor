...
8a. When a chart/figure has multiple data series (grouped bars, multiple line/scatter series, or multiple points along an axis) and EACH series carries a printed numeric label directly on the chart, every single labeled series/bar/point must become its own `performance.*` record -- not only the ones ALSO restated in body text. A series that exists ONLY as a chart label, never mentioned in prose, is still real data and must not be silently dropped.
    Self-check before finalizing: count the number of distinct series/bars/points visible in the figure that carry a printed numeric label, and confirm your extraction has exactly one record per labeled series for that figure. If your extracted count is lower than the labeled-series count, you have dropped data -- go back and add the missing records before finalizing.
    This self-check is NOT satisfied by a prose percentage claim describing the same figure (e.g. "wear rate decreased by 92%") -- that sentence is its own separate record under rule 9a below, extracted in ADDITION to this figure's plotted absolute values, never instead of them. If you have already written a percentage record for a figure, that is not a reason to skip counting and extracting its series here; do both.

9. Every field under `performance` ({{PACK_METRICS_INLINE}}) is a LIST OF STRUCTURED RECORDS, not a list of sentences. For each numeric fact, create ONE record with:
   ...
   - `confidence`: "confirmed" if the value is stated verbatim in text/table, OR is a number printed as text directly on the chart (a data label attached to that specific bar/point/series) -- use the exact printed value, never round it. "approximate" applies ONLY when you are visually interpolating between gridlines because the chart has no printed label at that point -- never mark a printed data label as "approximate" just because it came from a figure rather than a table. "derived" if the value is CALCULATED from a stated percentage/ratio and a stated baseline rather than read directly -- see rule 9a for the full requirement; never label a calculated value "confirmed". "not_reported" if you could not find or read the value at all (and in that case set `value` to "Not Reported").
...
9a. Numeric claims stated ONLY in body prose -- not just tables/figures -- must be scanned for and extracted with the same rigor as table values. Scan EVERY paragraph of body prose for this, not just the abstract or conclusion -- a single paper commonly restates results in prose in MULTIPLE separate sentences (e.g. one sentence for wear-rate reduction, a different sentence elsewhere for coefficient-of-friction reduction). Find and extract ALL of them, not just the first or most prominent one you notice.
    This especially includes relative claims tied to a named material: percentage reductions/increases, fold-changes, or comparative statements (e.g. "WR decreased by 16.3%, 23.8%, and 49.5% for X, Y, and Z respectively" or "the friction coefficient dropped by 40% compared to neat PPS"). Do not skip these because they are "just" a sentence instead of a table row.
    APPLY THE SAME ANTI-COLLAPSING RULE AS RULE 9 ABOVE, but for prose: when ONE sentence enumerates multiple values against multiple named items -- via "respectively", a comma-separated list, or repeated clauses -- that ONE sentence produces ONE record PER value, aligned in order to its named material. The example above ("WR decreased by 16.3%, 23.8%, and 49.5% for X, Y, and Z respectively") is THREE separate records, not one. Never summarize such a list down to a single representative/most-prominent value and drop the rest -- that is the same collapsing mistake rule 9 already forbids for tables, just happening in a sentence instead of a row.
    Each such claim becomes its own record under the correct `performance.*` field: `material` = the named material, `condition` = the test condition it was measured/compared under (best available from context), `value` = the number with its sign/percent exactly as written (e.g. "16.3%", "-40%"), `unit` = "%" (or the stated unit), `source` = "text".
    ...
    This SAME contradiction rule applies when the SAME comparison (same two materials/items, same metric) is stated with two DIFFERENT percentages in two different locations of the paper -- e.g. the abstract states "50% lower" while the results section states "47.9% lower" for the same pair. Do not silently pick one location and drop the other. Keep BOTH records (each `source` naming where it came from, e.g. "text (abstract)" / "text (Section 3.2)") and set `confidence: "contradicts_table"` on both -- this flag means "this record disagrees with another record for the same comparison," not specifically "disagrees with a table"; it applies identically whether the disagreement is prose-vs-table or prose-vs-prose.
    If a value is CALCULATED rather than read directly -- e.g. the paper states a percentage/ratio comparison ("2.5E-5, which is 50% lower than PPS@PDA-CNTs-SiC") and you compute the implied baseline (PPS@PDA-CNTs-SiC = 5.0E-5) -- that computed value is NOT "confirmed", even though the arithmetic is simple and the inputs are real. Set `confidence: "derived"` and make `source` name exactly what it was derived from, e.g. `source: "derived from stated 50% reduction relative to PPS@PDA-CNTs-SiC (2.50E-5)"`. "confirmed" is reserved for a value that appears as literal text/table/chart-label content -- never for a number you calculated yourself. This applies even when TWO stated percentages for the same comparison would each imply a different baseline -- in that case you have both a "derived" disagreement AND the contradiction rule above applies: keep both derived records, tagged `confidence: "contradicts_table"` instead of "derived" if their computed values disagree, since the disagreement is the more important signal to surface.
    A prose percentage-reduction claim and the chart-plotted absolute values it is describing are TWO SEPARATE, BOTH-REQUIRED records, never one substituting for the other. Finding "wear rate decreased by 92%" in the text does not excuse skipping that same figure's per-material absolute readings under rule 8/8a -- extract the percentage record per this rule AND go read the figure for its plotted values. Confirmed real regression: a paper stating three prose percentage reductions (e.g. -18.1%, -94.4%, -92%) for a multi-series wear/friction figure came back with only those three confirmed text records -- all eight of the figure's own plotted absolute values (approximate, source: Figure N) were silently dropped. A percentage claim describing a figure is a SIGNAL that figure has extractable absolute data, not a reason to stop looking for it.
